...
model_monthsfollow<-function(df){
  
  lm(serum_hemoglobin~months_of_follow_up, df)
  
}



##adding stats to each nested dataset
#stage

<-map(by_stage$stage_data, model_stage)
#age
...
#stage_plot
a<-unnest(by_stage, cols=c(stage_data
)) %>% names()

plot_stage<-by_stage %>% 
  mutate(plot = pmap(stage_data,
                     ~ggplot(data=.x, aes(stage, a))+
                       geom_jitter()))

#for loop for iterating over variables
...
print(plot_stage$plot)

print(plot_stage$plot)
ggsave("plot_stage.png")

unnest(plot_stage, stage_data)

#age_plot

plot_age<-by_age %>% 
  mutate(gg1 = map(age_data, ~ggplot(data=.x, aes(age, months_of_follow_up))+geom_jitter()),
         gg2= map(age_data, ~ggplot(data=.x, aes(age,systolic_bp))+geom_jitter()),
         g=pmap(list(gg1, gg2), ~facet_wrap(ncol = 3)))
plot_age
by_age
...
plot_monthsfollow<-by_monthsfollow %>% 
  mutate(plot = map(monthsfollow_data, ~ggplot(data=.x, aes(diastolic_bp, systolic_bp))+
                      geom_jitter()))


print(plot_monthsfollow$plot)

#by weight index


#building model
j=c(names(prostate_data_clean))


modelling<-function(i){
  
  lm(age~i, data =prostate_data_clean)
  
}
modelling(j)

output<-vector("list", length(j))
?vector

for(i in seq_along(j)){
  output[[i]]<-lm(age~ j[[i]], data =prostate_data_clean)
...
#created model and mapped the model to each list. 
staged<-stage%>% mutate(model= map(by_stage, diastolic_bp ))
staged<-stage %>% mutate(resids=map2(data, model, add_residuals))
staged
